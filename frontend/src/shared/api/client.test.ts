...
describe("API Client", () => {
  ...
  it("appends query parameters to the URL", async () => {
    ...
    await api.get("/workspaces", {
      params: { limit: 10, search: "test", empty: null },
    });
  ...
  it("attaches Authorization header when token provider is set", async () => {
    ...
    const callHeaders = vi.mocked(globalThis.fetch).mock.calls[0][1]
      ?.headers as Headers;
  ...
  it.each([401, 403, 404, 409])(
    "parses API.md standard error shape on %i responses",
    async (status) => {
      const errorPayload = {
        error: {
          code: `HTTP_${status}`,
      ...
      try {
        ...
        expect(apiError.code).toBe(`HTTP_${status}`);
        expect(apiError.message).toBe(`Request failed with status ${status}`);
  ...
  it("parses validation error details on 422 Unprocessable Entity", async () => {
    ...
    await expect(
      api.post("/auth/signup", { email: "invalid" }),
